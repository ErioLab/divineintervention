...
game.import("mode", function (lib, game, ui, get, ai, _status) {
	...
	(() => {
		...
		lib.element.player.say = function (str) {
			...
			}, ui.create.danmu, '', this.nickname, str)
			str = str.replace(/##assetURL##/g, lib.assetURL);
			ui.create.danmu('', this.nickname, str)
	...
	(() => {
		...
		const lmsc = lib.message.server.chat
		const lmse = lib.message.server.emotion
		//注入新的初始化函数
		lib.message.server.init = function (version, config, banned_info) {
			...
			if (!_status.waitingForPlayer) {
				...
				this.send(ui.create.danmu, '<span style="color:black;">', '提示', '本房允许旁观发言</span>')
			}
			this.send(() => {
				game.send = function () {
		...
		lib.message.server.chat = function (id, str) {
			if (lib.node.observing.includes(this)) {
				...
				}, ui.create.danmu, '<span style="color:green;">（旁观）', name, str + "</span>")
				return
			};
			lmsc.call(this, id, str)
		}
		lib.message.server.emotion = function (id, pack, emotion, name) {
			if (lib.node.observing.includes(this)) {
			...
			lmse.call(this, id, pack, emotion, name)
		}
	})();
	return {
		name: "doudizhu",
			...
			chooseCharacterOL: function () {
				...
				next.setContent(function () {
					...
					for (var i = 0; i < game.players.length; i++) {
						...
						list.push([
							game.players[i],
							[str, [event.list.randomRemove(num + num3 + 2), "characterx"]],
							selectButton,
							true,
						]);
					}
